...
double Distance((int x, int y, int z) point1, (int x, int y, int z)point2) // вычесление диагонали
{
    double dist = Math.Sqrt(Math.Pow((point2.x - point1.x), 2) + Math.Pow((point2.y - point1.y), 2) + Math.Pow((point2.z - point1.z), 2));
    return dist;
}

var pointA = GetPointCoords("Введите пространственные координаты точки А");
